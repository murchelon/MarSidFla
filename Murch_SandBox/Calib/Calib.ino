...
int setCalibrationMatrix( tsPoint_t * displayPtr, tsPoint_t * screenPtr, tsMatrix_t * matrixPtr)
{
  ...
  else
  {
    ...
    Serial.println(String("CFG_EEPROM_TOUCHSCREEN_CAL_AN = ") + String(matrixPtr->An));
    Serial.println(String("CFG_EEPROM_TOUCHSCREEN_CAL_BN = ") + String(matrixPtr->Bn));
    Serial.println(String("CFG_EEPROM_TOUCHSCREEN_CAL_CN = ") + String(matrixPtr->Cn));
    Serial.println(String("CFG_EEPROM_TOUCHSCREEN_CAL_DN = ") + String(matrixPtr->Dn));
    Serial.println(String("CFG_EEPROM_TOUCHSCREEN_CAL_EN = ") + String(matrixPtr->En));
    Serial.println(String("CFG_EEPROM_TOUCHSCREEN_CAL_FN = ") + String(matrixPtr->Fn));
    Serial.println(String("CFG_EEPROM_TOUCHSCREEN_CAL_DIVIDER = ") + String(matrixPtr->Divider));
    Serial.println(String("CFG_EEPROM_TOUCHSCREEN_CALIBRATED = ") + String("1"));
    Serial.println(String("------"));


	// CFG_EEPROM_TOUCHSCREEN_CAL_AN = -340480
	// CFG_EEPROM_TOUCHSCREEN_CAL_BN = 2240
	// CFG_EEPROM_TOUCHSCREEN_CAL_CN = 30880640
	// CFG_EEPROM_TOUCHSCREEN_CAL_DN = 1536
	// CFG_EEPROM_TOUCHSCREEN_CAL_EN = -205824
	// CFG_EEPROM_TOUCHSCREEN_CAL_FN = 11935104
	// CFG_EEPROM_TOUCHSCREEN_CAL_DIVIDER = -380184
	// CFG_EEPROM_TOUCHSCREEN_CALIBRATED = 1
...
void setup() 
{
  ...
  //tsCalibrate();  
	...
	_tsMatrix.An = -340480;
	_tsMatrix.Bn = 2240;
	_tsMatrix.Cn = 30880640;
	_tsMatrix.Dn = 1536;
	_tsMatrix.En = -205824;
	_tsMatrix.Fn = 11935104;
	_tsMatrix.Divider = -380184;
...
void loop() 
{
  ...
  calibrateTSPoint(&calibrated, &raw, &_tsMatrix );
	...
	Serial.println(String(calibrated.x) + String(" -- ") + calibrated.y);

  /* Draw a single pixel at the calibrated point */
  tft.fillCircle(calibrated.x, calibrated.y, 2, RA8875_BLACK);
}
